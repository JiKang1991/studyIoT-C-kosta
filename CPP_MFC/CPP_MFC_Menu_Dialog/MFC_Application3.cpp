...
// MFC_Application3.cpp: 애플리케이션에 대한 클래스 동작을 정의합니다.
//
...
#include "MFC_Application3Doc.h"
#include "MFC_Application3View.h"
...
BEGIN_MESSAGE_MAP(CMFCApplication3App, CWinApp)
	...
	ON_COMMAND(ID_DialogTest, &CMFCApplication3App::OnDialogtest)
	ON_COMMAND(ID_DLG_Review, &CMFCApplication3App::OnDlgReview)
END_MESSAGE_MAP()
...
void CMFCApplication3App::OnDialogtest()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	CDlgTest testDlg;
	if (testDlg.DoModal() == IDOK) {
		AfxMessageBox(L"OK 버튼이 눌렸습니다.");
	} else {
		AfxMessageBox(L"Cancel 버튼이 눌렸습니다.");
	}
	
}


void CMFCApplication3App::OnDlgReview()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	CDlgReview reviewDlg;
	if (reviewDlg.DoModal() == IDOK) {
		AfxMessageBox(L"OK 버튼이 눌렸습니다.");
	}
	else {
		AfxMessageBox(L"Cancel 버튼이 눌렸습니다.");
